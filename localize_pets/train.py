...
parser.add_argument("-dd", "--data_dir", default="/home/deepan/Downloads/",
                    type=str, help="Data directory for training")
...
parser.add_argument("--backbone", default="Resnet50", type=str,
                    help="Architecture name. Available options:"
                         " SimpleNet, EfficientDet, VGG19, Resnet50.")
parser.add_argument("-fe", "--feature_extraction", default=False, type=bool,
...
print("Samples in train and test dataset are %d and %d, respectively. "
      % (len(train_dataset), len(test_dataset)))
train_datagen = DataGenerator(train_dataset, config["batch_size"],
                              config["image_width"], config["image_height"],
                              transforms, True)
test_datagen = DataGenerator(test_dataset, 1, config["image_width"],
                             config["image_height"], transforms, True)

# Model build
architecturefactory = ArchitectureFactory(config["backbone"])
model_class = architecturefactory.factory()
model_class = model_class(backbone=config["backbone"],
...
# Model compile
model.compile(loss={"class_out": "categorical_crossentropy", "box_out": "mse"},
              optimizer=tf.keras.optimizers.Adam(learning_rate=1e-3),
              metrics={"class_out": "accuracy", "box_out": IOU(name="iou")})

# Training
model.fit(train_datagen, epochs=100,
          callbacks=[ShowTestImages(test_datagen),
                     tf.keras.callbacks.EarlyStopping(
                         monitor="box_out_iou", patience=3, mode="max"),
