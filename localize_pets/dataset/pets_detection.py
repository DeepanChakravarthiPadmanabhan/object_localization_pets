...
class Pets_Detection(Loader):
    ...
    Aids in loading RGB images, segmentation masks and detection bounding box information.
    # Arguments
        image_path:
        annotations_path:
        split:
    ...
    def __init__(self, data_dir, split, class_names='all'):
        ...
        if class_names == 'all':
            self.class_names = 1
        super(Pets_Detection, self).__init__(
            None, split, class_names, 'Pets')
    ...
    def read_xml(bbox_file):
        ...
        for member in root.findall('object'):
            value = {'filename': root.find('filename').text,  # filename
    ...
    def load_data(self):
        if self.split == 'trainval':
            ...
            for line in images_list:
                ...
                if (os.path.exists(os.path.join(self.xmls_dir, xml_name))
