...
from localize_pets.visualization.utils import deprocess_image
...
def initialize_image(width, height, normalize):
    ...
    if normalize == "max":
        ...
        raise ValueError("Normalization method unsupported %s" % normalize)
    ...
    for cur_row in range(rows):
        for cur_col in range(columns):
            if idx < num_images:
                ...
                idx += 1
    plt.tight_layout()
    plt.savefig("visualize_filter_" + layer_name + "_" + objective + ".jpg")
    plt.show()
